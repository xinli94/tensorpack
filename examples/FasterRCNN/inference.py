...
# from eval import DetectionResult, predict_image#, multithread_predict_dataflow #, detect_one_image, print_coco_metrics
from train import ResNetC4Model, ResNetFPNModel, do_visualize, do_predict
...
def _offline_evaluate(pred_config, output_file):
    ...
    for dataset in cfg.DATA.VAL:
        ...
        dataflows = [
            _get_eval_dataflow(dataset, shard=k, num_shards=num_gpu)
            for k in range(num_gpu)]
        all_results = _multithread_predict_dataflow(dataflows, graph_funcs)
...
def _get_eval_dataflow(name, shard=0, num_shards=1):
    """
    Args:
    ...
    # Note(xin): we need the image path
...
def _predict_dataflow(df, model_func, tqdm_bar=None):
    """
    Args:
    ...
    with ExitStack() as stack:
        ...
        for img, img_id, img_path in df:
            ...
            for r in results:
                res = {
                    # Note(xin): add `image_path` and `label`
                    'image_path': img_path,
                    'image_id': img_id,
                    'category_id': r.class_id,
                    'label': cfg.DATA.CLASS_NAMES[r.class_id],
                    'bbox': list(r.box),
...
def _multithread_predict_dataflow(dataflows, model_funcs):
    """
    ...
    if num_worker == 1:
        return _predict_dataflow(dataflows[0], model_funcs[0])
    with ThreadPoolExecutor(max_workers=num_worker, thread_name_prefix='EvalWorker') as executor, \
        ...
        for dataflow, pred in zip(dataflows, model_funcs):
            futures.append(executor.submit(_predict_dataflow, dataflow, pred, pbar))
